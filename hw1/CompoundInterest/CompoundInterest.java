...
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate,
                              int targetYear) {
        if (targetYear - THIS_YEAR == 0){
            return presentValue;
        }
        return futureValue(presentValue * (1 + rate/100), rate, targetYear - 1);
    }
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        if (targetYear == THIS_YEAR)
            return presentValue;
        return futureValueReal(presentValue * (1 + rate/100) * (1 - inflationRate/100), rate, targetYear - 1, inflationRate);
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        double savings = 0;
        double simpRate = 1.0 + rate/100;
        for (int i = 0; i <= targetYear - THIS_YEAR; i++){
            double temp = perYear;
            for (int j = 0; j < i; j++){
                temp *= simpRate;
            }
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
        ...
        double savings = totalSavings(perYear, targetYear, rate);
        double simpRate = 1.0 - inflationRate/100;
        for (int i = 0; i < targetYear - THIS_YEAR; i++){
            savings *= simpRate;
        }
        return savings;
