    ...
    Model(int[][] solution) {
            ...
            for (int j = 0; j < height(); j++) {
                if (_solution[i][j] == 1 || _solution[i][j] == last) {
                    _board[i][j] = new Sq(i, j,
                            _solution[i][j], true, arrowDirection(i, j), 0);
                } else {
                    _board[i][j] = new Sq(i, j,
        ...
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                _board[i][j]._successors = allSuccessors(
                        _board[i][j].pl, _board[i][j]._dir);
                _board[i][j]._predecessors = new PlaceList();
                for (Sq s: _allSquares) {
                    _board[i][j]._predecessors.add(s.pl);
                }
            }
        }
        _unconnected = last - 1;
        ...
        for (Sq s: _allSquares) {
            if (s.sequenceNum() != 0 && s._successor == null) {
                for (Place p: s.successors()) {
                    if (s.connectable(get(p)) && s.sequenceNum()
                            + 1 == get(p).sequenceNum()) {
                        s.connect(get(p));
                        connected = true;
                    }
    ...
    final class Sq {
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                ...
                boolean fixed = false; Sq temp = this;
                while (temp != null) {
                    if (temp.hasFixedNum()) {
                        fixed = true;
                    }
                ...
                if (!fixed) {
                    temp = this;
                    while (temp != null) {
                        temp._sequenceNum = 0; temp = temp.predecessor();
                    }
                ...
                if (!fixed) {
                    temp = next;
                    while (temp != null) {
                        temp._sequenceNum = 0; temp = temp.successor();
                    }
